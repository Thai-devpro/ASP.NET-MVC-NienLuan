...
namespace NienLuanCoSo.Areas.Admin.Controllers
{
    public class HienVatController : Controller
    {
        ...
        {
            var qg = db.TT_QUYENGOP_HIENVAT.Where(s => s.MA_HV == id);
            
            var tt = db.TT_TRAOTANG.Where(s => s.MA_HV == id);
            int qg1 = qg.Count();
            int tt1 = tt.Count();
            HIEN_VAT hv = db.HIEN_VAT.Find(id);
            if (qg1 > 0 || tt1 > 0)
            {
                ...
                return RedirectToAction("Loi", new { id = hv.MA_HV });
            }
            else
                return View(hv);

        }
        public ActionResult Loi(int id)
        {
            HIEN_VAT ttqg = db.HIEN_VAT.SingleOrDefault(s => s.MA_HV == id);
            return View(ttqg);
        }
