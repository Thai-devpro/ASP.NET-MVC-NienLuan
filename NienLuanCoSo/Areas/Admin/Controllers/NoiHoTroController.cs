...
namespace NienLuanCoSo.Areas.Admin.Controllers
{
    public class NoiHoTroController : Controller
    {
        ...
        public ActionResult Index(string Search = "")
        {
            ...
            {
                var top =
                (from p in db.NOIHOTROes
                 let totalQuantity = (from op in db.TT_TRAOTANG

                                      where op.MANOI == p.MANOI
                                      select op.MANOI).Sum()
            ...
            if (Search == "top10thang")
            {
                ...
                (from p in db.NOIHOTROes
                 let totalQuantity = (from op in db.TT_TRAOTANG
                                      ...
                                      where op.MANOI == p.MANOI && op.NGAYTANG.Month == DateTime.Now.Month
                                      select op.MANOI).Sum()
                 where totalQuantity > 0
                 orderby totalQuantity descending
                 select p).Take(10).ToList();
            ...
            if (Search == "top10nam")
            {
                ...
                (from p in db.NOIHOTROes
                 let totalQuantity = (from op in db.TT_TRAOTANG
                                      ...
                                      where op.MANOI == p.MANOI && op.NGAYTANG.Year == DateTime.Now.Year
                                      select op.MANOI).Sum()
                 where totalQuantity > 0
                 orderby totalQuantity descending
                 select p).Take(10).ToList();
            ...
            if (Search == "all")
            {
                ...
                var listNHT2= from s in db.NOIHOTROes  select s;
                return View(listNHT2);
            }
            var listNHT = from s in db.NOIHOTROes  select s;
            return View(listNHT);
        }
